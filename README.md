...
Library using the LDG system and the mbedTLS 3.6.5 library.
...
- get [mbedtls.ldg.r5.zip](https://ptonthat.fr/files/polarssl/sources/mbedtls.ldg.r5.zip) and unpack the contents of /mbedtls.ldg/ to  
   ```./README.md```  
   ```./Makefile```  
   ```./main.c```  
   ```./transport.h```  
...
- mbedtls.ldg.xcodeproj is for Xcode 26.1, you may not need it if you use something else.
